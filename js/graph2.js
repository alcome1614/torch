...
$(document).ready(function() {
    console.log('INITIALISATION 2');

   //
    var nodeMinimumDegreeList2 = [1, 5,10,20,50,100];
   //
   //
   ...
   d3.select("#selectButton2").on("change", function(d) {
    ...
    console.log('select button', selectedOption2);
    remove();
    draw2(selectedOption2);
...
function draw2(minimumDegree){
    // d3.select("body").select('#viz2').selectAll('svg').remove()
    console.log('REMOVE')
    console.log('DRAW')
     var strengthValue = 5;
    ...
    async function loadData(){
        ...
        const data = await d3.json('graphs/collaboration.json');
    ...
    loadData().then(() => {console.log('then 2');});
    ...
    for (const property in node){
     if (['name','id','DOI','deg','community','weight'].includes(property)){
         if (property=='DOI'){
             ...
             var doiInfoDiv = d3.select("#info").append("p").text(property +":")
